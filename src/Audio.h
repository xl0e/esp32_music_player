...
typedef std::function<void()> AudioEofCallback;
typedef std::function<void(int16_t, const char *)> AudioMetadataCallback;
...
public:
    ...
    void setAudioEofCallback(std::function<void()> _cb)
    {
        audio_eof_callback = _cb;
    }

    void setAudioId3DataCallback(std::function<void(uint16_t, const char *)> _cb)
    {
        audio_metadata_callback = _cb;
    }

private:
    std::function<void()> audio_eof_callback = nullptr;
    std::function<void(const char *)> audio_info = nullptr;
    std::function<void(const char *)> audio_id3data = nullptr;
    std::function<void(uint16_t, const char *)> audio_metadata_callback = [](uint16_t id, const char *data) {};
    std::function<void(const char *)> audio_lasthost = nullptr;
    std::function<void(const char *)> audio_eof_speech = nullptr;
    std::function<void(const char *)> audio_eof_stream = nullptr;
    std::function<void(const char *)> audio_showstation = nullptr;
    std::function<void(const char *)> audio_showstreamtitle = nullptr;
    std::function<void(const char *)> audio_icydescription = nullptr;
    std::function<void(const char *)> audio_icyurl = nullptr;
    std::function<void(const char *)> audio_icylogo = nullptr;
    std::function<void(const char *)> audio_bitrate = nullptr;
    std::function<void(const char *)> audio_commercial = nullptr;
    std::function<void(fs::File &, uint32_t, uint32_t)> audio_id3image = nullptr;
    std::function<void(fs::File &, std::vector<uint32_t>)> audio_oggimage = nullptr;
    std::function<void(fs::File &, uint32_t, size_t)> audio_id3lyrics = nullptr;
    std::function<void(int16_t *, int16_t, int, int, bool)> audio_process_i2s = nullptr;
...
private:
    ...
    void trim(char *str)
    {
        ...
        while (isspace((unsigned char)*start))
            start++; // find the first non-space character
    ...
    {
        if (!str)
            return strdup(""); // better not to return NULL
        char *ps_str = NULL;
        if (psramFound())
        {
