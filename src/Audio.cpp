...
Audio::Audio(bool internalDAC /* = false */, uint8_t channelEnabled /* = I2S_SLOT_MODE_STEREO */, uint8_t i2sPort) {
    ...
    m_f_Log = true;
#endif
#define AUDIO_INFO(...) log_i(__VA_ARGS__);
...
void Audio::showID3Tag(const char *tag, const char *value)
{
    ...
    // V2.2
    if (!strcmp(tag, "CNT"))
        audio_metadata_callback(ID3Tag::CNT, value); // sprintf(m_chbuf, "Play counter: %s", value);
    if (!strcmp(tag, "COM"))
        audio_metadata_callback(ID3Tag::COM, value); // sprintf(m_chbuf, "Comments: %s", value);
    if (!strcmp(tag, "CRA"))
        audio_metadata_callback(ID3Tag::CRA, value); // sprintf(m_chbuf, "Audio encryption: %s", value);
    if (!strcmp(tag, "CRM"))
        audio_metadata_callback(ID3Tag::CRM, value); // sprintf(m_chbuf, "Encrypted meta frame: %s", value);
    if (!strcmp(tag, "ETC"))
        audio_metadata_callback(ID3Tag::ETC, value); // sprintf(m_chbuf, "Event timing codes: %s", value);
    if (!strcmp(tag, "EQU"))
        audio_metadata_callback(ID3Tag::EQU, value); // sprintf(m_chbuf, "Equalization: %s", value);
    if (!strcmp(tag, "IPL"))
        audio_metadata_callback(ID3Tag::IPL, value); // sprintf(m_chbuf, "Involved people list: %s", value);
    if (!strcmp(tag, "PIC"))
        audio_metadata_callback(ID3Tag::PIC, value); // sprintf(m_chbuf, "Attached picture: %s", value);
    if (!strcmp(tag, "SLT"))
        audio_metadata_callback(ID3Tag::SLT, value); // sprintf(m_chbuf, "Synchronized lyric/text: %s", value);
    if (!strcmp(tag, "TAL"))
        audio_metadata_callback(ID3Tag::TAL, value); // sprintf(m_chbuf, "Album/Movie/Show title: %s", value);
    if (!strcmp(tag, "TBP"))
        audio_metadata_callback(ID3Tag::TBP, value); // sprintf(m_chbuf, "BPM (Beats Per Minute): %s", value);
    if (!strcmp(tag, "TCM"))
        audio_metadata_callback(ID3Tag::TCM, value); // sprintf(m_chbuf, "Composer: %s", value);
    if (!strcmp(tag, "TCO"))
        audio_metadata_callback(ID3Tag::TCO, value); // sprintf(m_chbuf, "Content type: %s", value);
    if (!strcmp(tag, "TCR"))
        audio_metadata_callback(ID3Tag::TCR, value); // sprintf(m_chbuf, "Copyright message: %s", value);
    if (!strcmp(tag, "TDA"))
        audio_metadata_callback(ID3Tag::TDA, value); // sprintf(m_chbuf, "Date: %s", value);
    if (!strcmp(tag, "TDY"))
        audio_metadata_callback(ID3Tag::TDY, value); // sprintf(m_chbuf, "Playlist delay: %s", value);
    if (!strcmp(tag, "TEN"))
        audio_metadata_callback(ID3Tag::TEN, value); // sprintf(m_chbuf, "Encoded by: %s", value);
    if (!strcmp(tag, "TFT"))
        audio_metadata_callback(ID3Tag::TFT, value); // sprintf(m_chbuf, "File type: %s", value);
    if (!strcmp(tag, "TIM"))
        audio_metadata_callback(ID3Tag::TIM, value); // sprintf(m_chbuf, "Time: %s", value);
    if (!strcmp(tag, "TKE"))
        audio_metadata_callback(ID3Tag::TKE, value); // sprintf(m_chbuf, "Initial key: %s", value);
    if (!strcmp(tag, "TLA"))
        audio_metadata_callback(ID3Tag::TLA, value); // sprintf(m_chbuf, "Language(s): %s", value);
    if (!strcmp(tag, "TLE"))
        audio_metadata_callback(ID3Tag::TLE, value); // sprintf(m_chbuf, "Length: %s", value);
    if (!strcmp(tag, "TMT"))
        audio_metadata_callback(ID3Tag::TMT, value); // sprintf(m_chbuf, "Media type: %s", value);
    if (!strcmp(tag, "TOA"))
        audio_metadata_callback(ID3Tag::TOA, value); // sprintf(m_chbuf, "Original artist(s)/performer(s): %s", value);
    if (!strcmp(tag, "TOF"))
        audio_metadata_callback(ID3Tag::TOF, value); // sprintf(m_chbuf, "Original filename: %s", value);
    if (!strcmp(tag, "TOL"))
        audio_metadata_callback(ID3Tag::TOL, value); // sprintf(m_chbuf, "Original Lyricist(s)/text writer(s): %s", value);
    if (!strcmp(tag, "TOR"))
        audio_metadata_callback(ID3Tag::TOR, value); // sprintf(m_chbuf, "Original release year: %s", value);
    if (!strcmp(tag, "TOT"))
        audio_metadata_callback(ID3Tag::TOT, value); // sprintf(m_chbuf, "Original album/Movie/Show title: %s", value);
    if (!strcmp(tag, "TP1"))
        audio_metadata_callback(ID3Tag::TP1, value); // sprintf(m_chbuf, "Lead artist(s)/Lead performer(s)/Soloist(s)/Performing group: %s", value);
    if (!strcmp(tag, "TP2"))
        audio_metadata_callback(ID3Tag::TP2, value); // sprintf(m_chbuf, "Band/Orchestra/Accompaniment: %s", value);
    if (!strcmp(tag, "TP3"))
        audio_metadata_callback(ID3Tag::TP3, value); // sprintf(m_chbuf, "Conductor/Performer refinement: %s", value);
    if (!strcmp(tag, "TP4"))
        audio_metadata_callback(ID3Tag::TP4, value); // sprintf(m_chbuf, "Interpreted, remixed, or otherwise modified by: %s", value);
    if (!strcmp(tag, "TPA"))
        audio_metadata_callback(ID3Tag::TPA, value); // sprintf(m_chbuf, "Part of a set: %s", value);
    if (!strcmp(tag, "TPB"))
        audio_metadata_callback(ID3Tag::TPB, value); // sprintf(m_chbuf, "Publisher: %s", value);
    if (!strcmp(tag, "TRC"))
        audio_metadata_callback(ID3Tag::TRC, value); // sprintf(m_chbuf, "ISRC (International Standard Recording Code): %s", value);
    if (!strcmp(tag, "TRD"))
        audio_metadata_callback(ID3Tag::TRD, value); // sprintf(m_chbuf, "Recording dates: %s", value);
    if (!strcmp(tag, "TRK"))
        audio_metadata_callback(ID3Tag::TRK, value); // sprintf(m_chbuf, "Track number/Position in set: %s", value);
    if (!strcmp(tag, "TSI"))
        audio_metadata_callback(ID3Tag::TSI, value); // sprintf(m_chbuf, "Size: %s", value);
    if (!strcmp(tag, "TSS"))
        audio_metadata_callback(ID3Tag::TSS, value); // sprintf(m_chbuf, "Software/hardware and settings used for encoding: %s", value);
    if (!strcmp(tag, "TT1"))
        audio_metadata_callback(ID3Tag::TT1, value); // sprintf(m_chbuf, "Content group description: %s", value);
    if (!strcmp(tag, "TT2"))
        audio_metadata_callback(ID3Tag::TT2, value); // sprintf(m_chbuf, "Title/Songname/Content description: %s", value);
    if (!strcmp(tag, "TT3"))
        audio_metadata_callback(ID3Tag::TT3, value); // sprintf(m_chbuf, "Subtitle/Description refinement: %s", value);
    if (!strcmp(tag, "TXT"))
        audio_metadata_callback(ID3Tag::TXT, value); // sprintf(m_chbuf, "Lyricist/text writer: %s", value);
    if (!strcmp(tag, "TXX"))
        audio_metadata_callback(ID3Tag::TXX, value); // sprintf(m_chbuf, "User defined text information frame: %s", value);
    if (!strcmp(tag, "TYE"))
        audio_metadata_callback(ID3Tag::TYE, value); // sprintf(m_chbuf, "Year: %s", value);
    if (!strcmp(tag, "UFI"))
        audio_metadata_callback(ID3Tag::UFI, value); // sprintf(m_chbuf, "Unique file identifier: %s", value);
    if (!strcmp(tag, "ULT"))
        audio_metadata_callback(ID3Tag::ULT, value); // sprintf(m_chbuf, "Unsychronized lyric/text transcription: %s", value);
    if (!strcmp(tag, "WAF"))
        audio_metadata_callback(ID3Tag::WAF, value); // sprintf(m_chbuf, "Official audio file webpage: %s", value);
    if (!strcmp(tag, "WAR"))
        audio_metadata_callback(ID3Tag::WAR, value); // sprintf(m_chbuf, "Official artist/performer webpage: %s", value);
    if (!strcmp(tag, "WAS"))
        audio_metadata_callback(ID3Tag::WAS, value); // sprintf(m_chbuf, "Official audio source webpage: %s", value);
    if (!strcmp(tag, "WCM"))
        audio_metadata_callback(ID3Tag::WCM, value); // sprintf(m_chbuf, "Commercial information: %s", value);
    if (!strcmp(tag, "WCP"))
        audio_metadata_callback(ID3Tag::WCP, value); // sprintf(m_chbuf, "Copyright/Legal information: %s", value);
    if (!strcmp(tag, "WPB"))
        audio_metadata_callback(ID3Tag::WPB, value); // sprintf(m_chbuf, "Publishers official webpage: %s", value);
    if (!strcmp(tag, "WXX"))
        audio_metadata_callback(ID3Tag::WXX, value); // sprintf(m_chbuf, "User defined URL link frame: %s", value);

    // V2.3 V2.4 tags
    if (!strcmp(tag, "COMM"))
        audio_metadata_callback(ID3Tag::COMM, value); // sprintf(m_chbuf, "Comment: %s", value);
    if (!strcmp(tag, "OWNE"))
        audio_metadata_callback(ID3Tag::OWNE, value); // sprintf(m_chbuf, "Ownership: %s", value);
    if (!strcmp(tag, "PRIV"))
        audio_metadata_callback(ID3Tag::PRIV, value); // sprintf(m_chbuf, "Private: %s", value);
    if (!strcmp(tag, "SYLT"))
        audio_metadata_callback(ID3Tag::SYLT, value); // sprintf(m_chbuf, "SynLyrics: %s", value);
    if (!strcmp(tag, "TALB"))
        audio_metadata_callback(ID3Tag::TALB, value); // sprintf(m_chbuf, "Album: %s", value);
    if (!strcmp(tag, "TBPM"))
        audio_metadata_callback(ID3Tag::TBPM, value); // sprintf(m_chbuf, "BeatsPerMinute: %s", value);
    if (!strcmp(tag, "TCMP"))
        audio_metadata_callback(ID3Tag::TCMP, value); // sprintf(m_chbuf, "Compilation: %s", value);
    if (!strcmp(tag, "TCOM"))
        audio_metadata_callback(ID3Tag::TCOM, value); // sprintf(m_chbuf, "Composer: %s", value);
    if (!strcmp(tag, "TCON"))
        audio_metadata_callback(ID3Tag::TCON, value); // sprintf(m_chbuf, "ContentType: %s", value);
    if (!strcmp(tag, "TCOP"))
        audio_metadata_callback(ID3Tag::TCOP, value); // sprintf(m_chbuf, "Copyright: %s", value);
    if (!strcmp(tag, "TDAT"))
        audio_metadata_callback(ID3Tag::TDAT, value); // sprintf(m_chbuf, "Date: %s", value);
    if (!strcmp(tag, "TEXT"))
        audio_metadata_callback(ID3Tag::TEXT, value); // sprintf(m_chbuf, "Lyricist: %s", value);
    if (!strcmp(tag, "TIME"))
        audio_metadata_callback(ID3Tag::TIME, value); // sprintf(m_chbuf, "Time: %s", value);
    if (!strcmp(tag, "TIT1"))
        audio_metadata_callback(ID3Tag::TIT1, value); // sprintf(m_chbuf, "Grouping: %s", value);
    if (!strcmp(tag, "TIT2"))
        audio_metadata_callback(ID3Tag::TIT2, value); // sprintf(m_chbuf, "Title: %s", value);
    if (!strcmp(tag, "TIT3"))
        audio_metadata_callback(ID3Tag::TIT3, value); // sprintf(m_chbuf, "Subtitle: %s", value);
    if (!strcmp(tag, "TLAN"))
        audio_metadata_callback(ID3Tag::TLAN, value); // sprintf(m_chbuf, "Language: %s", value);
    if (!strcmp(tag, "TLEN"))
        audio_metadata_callback(ID3Tag::TLEN, value); // sprintf(m_chbuf, "Length (ms): %s", value);
    if (!strcmp(tag, "TMED"))
        audio_metadata_callback(ID3Tag::TMED, value); // sprintf(m_chbuf, "Media: %s", value);
    if (!strcmp(tag, "TOAL"))
        audio_metadata_callback(ID3Tag::TOAL, value); // sprintf(m_chbuf, "OriginalAlbum: %s", value);
    if (!strcmp(tag, "TOPE"))
        audio_metadata_callback(ID3Tag::TOPE, value); // sprintf(m_chbuf, "OriginalArtist: %s", value);
    if (!strcmp(tag, "TORY"))
        audio_metadata_callback(ID3Tag::TORY, value); // sprintf(m_chbuf, "OriginalReleaseYear: %s", value);
    if (!strcmp(tag, "TPE1"))
        audio_metadata_callback(ID3Tag::TPE1, value); // sprintf(m_chbuf, "Artist: %s", value);
    if (!strcmp(tag, "TPE2"))
        audio_metadata_callback(ID3Tag::TPE2, value); // sprintf(m_chbuf, "Band: %s", value);
    if (!strcmp(tag, "TPE3"))
        audio_metadata_callback(ID3Tag::TPE3, value); // sprintf(m_chbuf, "Conductor: %s", value);
    if (!strcmp(tag, "TPE4"))
        audio_metadata_callback(ID3Tag::TPE4, value); // sprintf(m_chbuf, "InterpretedBy: %s", value);
    if (!strcmp(tag, "TPOS"))
        audio_metadata_callback(ID3Tag::TPOS, value); // sprintf(m_chbuf, "PartOfSet: %s", value);
    if (!strcmp(tag, "TPUB"))
        audio_metadata_callback(ID3Tag::TPUB, value); // sprintf(m_chbuf, "Publisher: %s", value);
    if (!strcmp(tag, "TRCK"))
        audio_metadata_callback(ID3Tag::TRCK, value); // sprintf(m_chbuf, "Track: %s", value);
    if (!strcmp(tag, "TSSE"))
        audio_metadata_callback(ID3Tag::TSSE, value); // sprintf(m_chbuf, "SettingsForEncoding: %s", value);
    if (!strcmp(tag, "TRDA"))
        audio_metadata_callback(ID3Tag::TRDA, value); // sprintf(m_chbuf, "RecordingDates: %s", value);
    if (!m_f_m3u8data)
        if (!strcmp(tag, "TXXX"))
            audio_metadata_callback(ID3Tag::TXXX, value); // sprintf(m_chbuf, "UserDefinedText: %s", value);
    if (!strcmp(tag, "TYER"))
        audio_metadata_callback(ID3Tag::TYER, value); // sprintf(m_chbuf, "Year: %s", value);
    if (!strcmp(tag, "USER"))
        audio_metadata_callback(ID3Tag::USER, value); // sprintf(m_chbuf, "TermsOfUse: %s", value);
    if (!strcmp(tag, "USLT"))
        audio_metadata_callback(ID3Tag::USLT, value); // sprintf(m_chbuf, "Lyrics: %s", value);
    if (!strcmp(tag, "WOAR"))
        audio_metadata_callback(ID3Tag::WOAR, value); // sprintf(m_chbuf, "OfficialArtistWebpage: %s", value);
    if (!strcmp(tag, "XDOR"))
        audio_metadata_callback(ID3Tag::XDOR, value); // sprintf(m_chbuf, "OriginalReleaseTime: %s", value);
...
int Audio::read_FLAC_Header(uint8_t *data, size_t len)
{
    ...
    { /* Stream info block */
        ...
        {
            AUDIO_INFO("total samples in stream: %lu", (long unsigned int)m_flacTotalSamplesInStream);
        }
        else
        {
        ...
        {
            AUDIO_INFO("audio file duration: %lu seconds", (long unsigned int)m_flacTotalSamplesInStream / (long unsigned int)m_flacSampleRate);
        }
        m_controlCounter = FLAC_MBH; // METADATA_BLOCK_HEADER
        retvalue = l + 3;
        headerSize += retvalue;
...
int Audio::read_ID3_Header(uint8_t *data, size_t len)
{
    ...
    { /* read ID3 tag and ID3 header size */
        ...
        { // ID3 not found
            if (!m_f_m3u8data)
                AUDIO_INFO("file has no ID3 tag, skip metadata");
    ...
    { //  exist another ID3tag?
        ...
        else
        {
            ...
            if (!m_f_m3u8data)
                AUDIO_INFO("Audio-Length: %u", m_audioDataSize);
            if (APIC_pos[0] && audio_id3image)
            { // if we have more than one APIC, output the first only
                size_t pos = audiofile.position();
            ...
            totalId3Size = 0;
            for (int i = 0; i < 3; i++)
                APIC_pos[i] = 0; // delete all
            for (int i = 0; i < 3; i++)
                APIC_size[i] = 0; // delete all
            return 0;
        }
    }
    return 0;
...
int Audio::read_M4A_Header(uint8_t *data, size_t len)
{
    ...
    { // trak
        if (specialIndexOf(data, "esds", len) > 0)
        {
            ...
            {
                AUDIO_INFO("unknown Audio Type %x", object_type_indication);
            }
            ...
            {
                ;
            } // log_w("Found  DecoderSpecificInfo Tag (Tag: 0x05)")
            pos++;
            ...
            {
                AUDIO_INFO("unknown ObjectType %x, stop", objectType);
                stopSong();
            }
            if (objectType < 7)
        ...
        if (!m_f_m4aID3dataAreRead)
        {
            for (int i = 0; i < 12; i++)
            {
                ...
                if (offset > 0)
                {
                    ...
                    m_chbuf[0] = '\0';
                    if (i == 0)
                        audio_metadata_callback(ID3Tag::TIT2, value);
                    if (i == 1)
                        audio_metadata_callback(ID3Tag::TPE1, value);
                    if (i == 2)
                        audio_metadata_callback(ID3Tag::TALB, value);
                    if (i == 3)
                        audio_metadata_callback(ID3Tag::TEN, value);
                    if (i == 4)
                        audio_metadata_callback(ID3Tag::COMM, value);
                    if (i == 5)
                        audio_metadata_callback(ID3Tag::TCOM, value);
                    if (i == 6)
                        audio_metadata_callback(ID3Tag::TBPM, value);
                    if (i == 7)
                        audio_metadata_callback(ID3Tag::TRCK, value);
                    if (i == 8)
                        audio_metadata_callback(ID3Tag::TYER, value);
                    if (i == 9)
                        audio_metadata_callback(ID3Tag::TCMP, value);
                    if (i == 10)
                        audio_metadata_callback(ID3Tag::TPE2, value);
                    if (i == 11)
                        audio_metadata_callback(ID3Tag::TCON, value);
                    if (m_chbuf[0] != 0)
                    {
                        if (audio_id3data)
                            audio_id3data(m_chbuf);
...
#endif
    // i2s_bytesConsumed = this->i2s->write((int8_t *)m_outBuff + 2 * count, 2 * validSamples * sampleSize);
        ...
        case AUDIO_PLAYLISTDATA:
            ...
            { // host == NULL means connect to m3u8 URL
                if (m_lastM3U8host)
                    ...
                    httpPrint(m_lastHost);         // if url has no first redirection
    ...
    for (int i = 0; i < lines; i++)
    {
        ...
        {                                             // Info?
            ...
            if (pos > 0)
            {
                ...
                char* str = m_playlistContent[i] + pos + 1;
                AUDIO_INFO("%s", str);
            }
            continue;
        }
    ...
    if (lines)
    {
        for (uint16_t i = 0; i < lines; i++)
        {
            ...
            if (m_codec == CODEC_NONE)
                m_codec = CODEC_AAC; // if we have no redirection
...
const char *Audio::m3u8redirection(uint8_t *codec)
{
    ...
    { // looking for lowest codeString
        ...
        if (posCodec > 0)
        {
            ...
            if (!found)
                log_w("codeString %s not in list", m_playlistContent[i] + posCodec);
        }
        if (cS == 0)
            *codec = CODEC_MP3;
        if (cS > 0 && cS < 100)
...
{
    if (!(audiofile && m_f_running && m_dataMode == AUDIO_LOCALFILE))
        return; // guard
    ...
    if (m_resumeFilePos >= 0)
    {
        ...
        if (m_codec == CODEC_VORBIS)
        {
            m_resumeFilePos = ogg_correctResumeFilePos(m_resumeFilePos);
            if (m_resumeFilePos == -1)
                goto exit;
        ...
        while (m_f_audioTaskIsDecoding)
            vTaskDelay(1); // We can't reset the InBuffer while the decoding is in progress
...
void Audio::processWebFile()
{
    ...
    if (m_controlCounter != 100)
    {
        if (InBuff.bufferFilled() > maxFrameSize || (InBuff.bufferFilled() == m_contentlength))
        { // at least one complete frame or the file is smaller
...
void Audio::showCodecParams()
{
    ...
    if (getBitRate())
    {
        AUDIO_INFO("BitRate: %lu", (long unsigned int)getBitRate());
    }
    else
    {
...
void Audio::setDecoderItems()
{
    if (m_codec == CODEC_MP3)
    {
        ...
        AUDIO_INFO("MPEG-%s, Layer %s", (MP3GetVersion() == 0) ? "2.5" : (MP3GetVersion() == 2) ? "2"
                                                                                                : "1",
                   (MP3GetLayer() == 1) ? "III" : (MP3GetLayer() == 2) ? "II"
                                                                       : "I");
    }
...
int Audio::sendBytes(uint8_t *data, size_t len)
{
    ...
    { // Error, skip the frame...
        ...
        if (m_codec == CODEC_OPUS)
        {
            if (m_decodeError == ERR_OPUS_HYBRID_MODE_UNSUPPORTED)
                stopSong();
            if (m_decodeError == ERR_OPUS_SILK_MODE_UNSUPPORTED)
                stopSong();
            if (m_decodeError == ERR_OPUS_NARROW_BAND_UNSUPPORTED)
                stopSong();
            if (m_decodeError == ERR_OPUS_WIDE_BAND_UNSUPPORTED)
                stopSong();
            if (m_decodeError == ERR_OPUS_SUPER_WIDE_BAND_UNSUPPORTED)
                stopSong();
            if (m_decodeError == ERR_OPUS_INVALID_SAMPLERATE)
                stopSong();
            return 0;
        }
    ...
    case CODEC_AAC:
        ...
        if (!isPS && AACGetParametricStereo())
        { // only change 0 -> 1
            isPS = 1;
            AUDIO_INFO("Parametric Stereo");
        }
        else
            isPS = AACGetParametricStereo();
        break;
    case CODEC_M4A:
        m_validSamples = AACGetOutputSamps() / getChannels();
        break;
...
void Audio::printProcessLog(int r, const char *s)
{
    ...
    if (logLevel == 1)
    {
        AUDIO_INFO("ERROR: %s%s", e, f);
    }
    else if (logLevel == 2)
    {
        AUDIO_INFO("WARNING: %s%s", e, f);
    }
    else
    {
        AUDIO_INFO("INFO: %s%s", e, f);
    }
}
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void Audio::printDecodeError(int r)
{
    ...
    if (m_codec == CODEC_FLAC)
    {
        ...
        case ERR_FLAC_BITS_PER_SAMPLE_TOO_BIG:
            ...
            break;
        case ERR_FLAC_BITS_PER_SAMPLE_UNKNOWN:
            e = "BITS PER SAMPLE UNKNOWN";
            break;
        case ERR_FLAC_DECODER_ASYNC:
            e = "DECODER ASYNCHRON";
            break;
        case ERR_FLAC_BITREADER_UNDERFLOW:
            e = "BITREADER ERROR";
            break;
        case ERR_FLAC_OUTBUFFER_TOO_SMALL:
    ...
    if (m_codec == CODEC_OPUS)
    {
        ...
        case ERR_OPUS_HYBRID_MODE_UNSUPPORTED:
            e = "HYBRID MODE UNSUPPORTED";
            break;
        case ERR_OPUS_NARROW_BAND_UNSUPPORTED:
            e = "NARROW_BAND_UNSUPPORTED";
            break;
        case ERR_OPUS_WIDE_BAND_UNSUPPORTED:
            e = "WIDE_BAND_UNSUPPORTED";
            break;
        case ERR_OPUS_SUPER_WIDE_BAND_UNSUPPORTED:
            e = "SUPER_WIDE_BAND_UNSUPPORTED";
            break;
...
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
// void Audio::setI2S(I2SStream &_i2s)
// {
//     i2s = &_i2s;
// }
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
{ // fast forward or rewind the current position in seconds
    if (/* m_streamType != ST_WEBFILE && */ m_dataMode != AUDIO_LOCALFILE)
        return false;
    if (m_dataMode == AUDIO_LOCALFILE && !audiofile)
        return false;
    if (!m_avr_bitrate)
        return false;
    if (m_codec == CODEC_AAC)
...
{ // curve 0: default, curve 1: flat at the beginning
    ...
    if (v < 207)
        AUDIO_INFO("Do not use this ancient Adruino version V%d.%d.%d", ESP_ARDUINO_VERSION_MAJOR, ESP_ARDUINO_VERSION_MINOR, ESP_ARDUINO_VERSION_PATCH);
...
bool Audio::ts_parsePacket(uint8_t *packet, uint8_t *packetStart, uint8_t *packetLength)
{
    ...
    if (PID == 0)
    {
        ...
        for (int i = startOfProgramNums; i <= sectionLength; i += lengthOfPATValue)
        {
            ...
            if (m_f_Log)
                log_i("Program Num: 0x%04X(%d) PMT PID: 0x%04X(%d)", program_number, program_number, program_map_PID, program_map_PID);
...
bool Audio::readID3V1Tag()
{
    ...
    { // maybe a V1.x TAG
        ...
        char track[] = {'\0','\0'};
        track[0] = *(InBuff.getReadPtr() + 126);
        char genre[] = {'\0','\0'};
        genre[0] = *(InBuff.getReadPtr() + 127);
        if (zeroByte)
        {
        ...
        if (strlen(title))
        {
            audio_metadata_callback(ID3Tag::TIT2, title);
        }
        if (strlen(artist))
        {
            audio_metadata_callback(ID3Tag::TPE1, artist);
        }
        if (strlen(album))
        {
            audio_metadata_callback(ID3Tag::TALB, album);
        }
        if (strlen(year))
        {
            audio_metadata_callback(ID3Tag::TYER, year);
        }
        if (strlen(comment))
        {
           audio_metadata_callback(ID3Tag::COMM, comment);
        }
        if (zeroByte == 0)
        {
            audio_metadata_callback(ID3Tag::TRCK, track);
        }
        if (genre[0] < 192)
        {
            audio_metadata_callback(ID3Tag::TCON, genre);
        } //[1]
        return true;
    }
...
uint32_t Audio::find_m4a_atom(uint32_t fileSize, const char *atomName, uint32_t depth)
{
    ...
    while (audiofile.position() < fileSize)
    {
        ...
        if (!atomSize)
            audiofile.read((uint8_t *)&atomSize, 4); // skip 4 byte offset field
...
uint32_t Audio::ogg_correctResumeFilePos(uint32_t resumeFilePos)
{
    ...
    if (found)
    {
        log_w("out_resumeFilePos %i", pos - 4);
        return (pos - 4);
    }
    stopSong();
    return 0;
...
int32_t Audio::mp3_correctResumeFilePos(uint32_t resumeFilePos)
{
    ...
    while (true)
    {
        ...
        if (brIdx)
        {
            ...
            if (MP3GetBitrate() == bitrate && getSampleRate() == samplerate)
                break;
        }
        pos++;
    }
...
{ // Recommendation:If the ARDUINO RUNNING CORE is 1, the audio task should be core 0 or vice versa
    if (coreID > 1)
        return;
    stopAudioTask();
    xSemaphoreTake(mutex_audioTask, 0.3 * configTICK_RATE_HZ);
...
void Audio::performAudioTask()
{
    ...
    if (!m_f_stream)
        return;
    if (m_codec == CODEC_NONE)
        return; // wait for codec is  set
    if (m_codec == CODEC_OGG)
        return; // wait for FLAC, VORBIS or OPUS
    xSemaphoreTake(mutex_audioTask, 0.3 * configTICK_RATE_HZ);
